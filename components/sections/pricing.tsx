    ...
    <section className="bg-black py-24 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-gray-500 uppercase tracking-widest text-sm">Quality that lasts a lifetime</p>
        </motion.div>
          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative p-8 rounded-2xl flex flex-col ${
                plan.highlight 
                  ? "bg-zinc-900 border-2 border-violet-500/50 shadow-[0_0_30px_rgba(139,92,246,0.15)] scale-105 z-10" 
              ...
              <ul className="space-y-4 mb-8">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-center text-sm text-gray-300">
                    <div className="w-1 h-1 bg-violet-500 rounded-full mr-3" />
                    {feature}
                  </li>
                ))}
              </ul>

              <button className={`w-full py-3 rounded-full text-sm tracking-widest uppercase transition-all ${
                plan.highlight 
